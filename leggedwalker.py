...
class LeggedAgent:
    ...
    def __init__(self):
        ...
        self.LegLength = 15
        self.footX = self.jointX + self.LegLength * math.sin(self.angle)     ##?
        self.footY = self.jointY + self.LegLength * math.cos(self.angle)     ##?
    ...
    def step3(self, stepsize, u, noise):  ## added argument noise ==> specifies noise array to add to motor output
        force = 0.0
        ...
        if (self.footstate == 1.0):
            ...
            self.omega = ((angle - self.angle)/stepsize) + noise
            self.angle = angle
        else:
            self.vx = 0.0
            self.omega = (self.omega + stepsize * MaxTorque * (self.backwardForce - self.forwardForce)) + noise
